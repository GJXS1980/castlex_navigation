...
class MoveBaseDoor():
    def __init__(self):
        ...
        #   订阅讲解话题
        self.sp_pub = rospy.Subscriber("/Commen_CMD_Topic", Int32, self.voice_com_order)
    ...
    #   获取yaml文件数据(data:yaml的文件路径，str_word:获取名称， i:提取几个导航点，j：选取的路径,k：路径有几个途经点 )
        self.routes = self.yaml_data(self.data, 'route', None, 1, 5, None)
        #   获取导航点
        self.waypoints = self.yaml_data(self.data, 'waypoint', 8, None, 5, None)
        #   获取语音文件
        self.sounds = self.yaml_data(self.data, 'sound', None, None, None, 10)
