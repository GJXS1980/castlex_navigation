...
class castlex_with_arm():
    ...
    #   获取yaml文件数据(data:yaml的文件路径，str_word:获取名称， i:提取几个导航点，j：选取的路径,k：路径有几个途经点 )
        self.routes = self.yaml_data(self.data, 'route', None, 4, 4, None)
        #   获取导航点
        ...
        if self.nav_data:
            ...
            for i in range(0, 2):
                if arm_data == 1:
                    while(self.front_id):
                        #   前进
                        self.castlex_odom_front(0.195, 0.01)
                    #   发布mqtt话题
                    self.arm_data = True
                    #rospy.sleep(1)
                    break
            ...
            while(arm_data):
                ...
                if self.sub_action == "loading complete" and self.sub_feedback == 1:
                    ...
                    while(self.back_id):
                        # 后退
                        self.castlex_odom_back(0.014, -0.01)
                    break
            self.nav_data = False
    ...
    def castlex_odom_front(self, data, data_vel): 
        move_cmd = Twist()
        while(self.front_id):
            if ((self.odom_front - self.odom_x) < data and (self.odom_front - self.odom_x) > 0) or ((self.odom_front - self.odom_x) > -(data) and (self.odom_front - self.odom_x) < 0):
                move_cmd.linear.x = data_vel
                self.cmd_vel.publish(move_cmd) 
                print((self.odom_front - self.odom_x))
            else:
                move_cmd.linear.x = 0.0
                self.cmd_vel.publish(move_cmd) 
                self.front_id = 0
                self.odom_back = self.odom_x

    #   里程计控制函数
    def castlex_odom_back(self, data, data_vel): 
        move_cmd = Twist()
        while(self.back_id):
            if ((self.odom_back - self.odom_x) < data and (self.odom_back - self.odom_x) > 0) or ((self.odom_back - self.odom_x) > -(data) and (self.odom_back - self.odom_x) < 0):
                move_cmd.linear.x = data_vel
                self.cmd_vel.publish(move_cmd) 
                print((self.odom_back - self.odom_x))
            else:
                move_cmd.linear.x = 0.0
                self.cmd_vel.publish(move_cmd) 
                self.back_id = 0
