...
class MoveBaseDoor():
    ...
    #   获取yaml文件数据(data:yaml的文件路径，str_word:获取名称， i:提取几个导航点，j：选取的路径,k：路径有几个途经点 )
        self.routes = self.yaml_data(self.data, 'route', None, 0, 7, None)
        #   获取导航点
        self.waypoints = self.yaml_data(self.data, 'waypoint', 8, None, None, None)
        #   获取语音文件
        self.sounds = self.yaml_data(self.data, 'sound', 8, None, None, 10)
    ...
    # 路径规划
    def routing(self, nav_data, sound_data1, sound_data2):
        self.goal(nav_data)
        if self.nav_data:
            for i in range(0, 10):
                if i == sound_data1:
                    playsound(self.sounds[i])
                    rospy.sleep(1)
                else:
                    pass
            for j in range(0, 10):
