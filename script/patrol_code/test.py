...
class MoveBaseDoor():
    ...
    # 路径规划
    def routing(self, data1, data2, data3, data4):
        if(self.i == data1 and self.dir==data2) :
            self.play_mp3(data3)
            rospy.sleep(1)
            self.play_mp3(data4)
    ...
    def move(self, goal):
        ...
        if not finished_within_time:
            ...
            if state == GoalStatus.SUCCEEDED: 
                self.routing(3, 0, 2, 3)
                self.routing(1, 0, 4, 5)
                self.routing(0, 0, 6, 7)
                self.routing(2, 0, 8, 1)
                rospy.sleep(1)
                #   发布导航成功的flag
